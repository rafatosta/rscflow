# Matriz de testes anterior ao fluxo simplificado

> Documento histórico arquivado em 13/09/2026. Contém referências a suítes removidas e não deve ser
> usado como instrução operacional. Consulte `../../testing.md` para a matriz vigente.
...
`tests/regulation/scoring.test.ts` fixa parâmetros conferidos nos arts. 12, 15 e 17 e recortes dos três níveis com proveniência em `tests/regulation/fixtures/resolution-excerpts.json`. `tests/regulation/catalog-integrity.test.ts` valida schema, IDs e códigos únicos, vínculos, valores não negativos, diretrizes esperadas, contagens 48/36/53, somas 100/10 e o conflito literal do RSC II d.5. Os resultados esperados são independentes da implementação. O catálogo preenchido continua indisponível para pontuação enquanto estiver pendente.
