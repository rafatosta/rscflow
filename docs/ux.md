...
O fluxo é Projetos → Visão geral → Dados do docente → Requisitos → Memorial → Revisão → Prévia de documentos → Gerar documentos. Formação integra Dados do docente; pontuação aparece como resultado e documentos são anexados dentro dos lançamentos. A prévia pode ser aberta pelo menu principal ou pela ação da Revisão.
