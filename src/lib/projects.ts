...
export async function deleteLocalProject(localId: string) { await db.transaction("rw", db.projects, db.attachments, async () => { await db.projects.delete(localId); await db.attachments.where("projectId").equals(localId).delete() }) }
