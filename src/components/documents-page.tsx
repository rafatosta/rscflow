...
export function DocumentsPage({ project, catalog }: { project: LocalProject; catalog?: Regulation }) {
  ...
  const items = [
    ...
    { id: "zip" as const, title: "Pacote final de documentos (.zip)", icon: FileArchive, ready: Boolean(project.identification && hasConclusion && catalog && occurrences.length && !noEvidence && storedAttachments.length), requirements: ["Memorial e formulários prontos", "Comprovantes binários disponíveis", "Mesma fotografia do projeto"], notes: ["Este pacote é destinado à entrega e reúne os PDFs, formulários e comprovantes organizados. Ele não substitui o backup do projeto.", !storedAttachments.length && "Sem comprovantes binários, o pacote final permanece bloqueado."].filter(Boolean) as string[] },
  ]
