...
export function BackupPage({ project }: { project: LocalProject }) {
  ...
  return <section className="mt-6 space-y-6">
    ...
    <div className="grid gap-4 lg:grid-cols-2"><BackupCard icon={FileArchive} title="Criar backup do projeto" description="Guarde uma cópia do projeto e dos arquivos salvos neste navegador." action={<Button onClick={() => void backup()}><Download /> Criar backup</Button>} />
      <BackupCard icon={Upload} title="Restaurar backup do projeto" description="Recupere uma cópia salva sem substituir o projeto atual." action={<Input type="file" accept=".rscflow" onChange={selectFile} />} />
    </div>
